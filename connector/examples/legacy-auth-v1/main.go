// Reference Legacy Authority Provider connector. Implements
// connector/provider/authority/v1 with an in-memory store. Demonstrates v1
// info + v1 health + WriteV1Error wiring for a v1-family provider whose
// function group code is "legacyAuthorityProvider" while the route paths
// use the literal "authorityProvider" path token.
//
// Run:
//
//	go run ./connector/examples/legacy-auth-v1
//
...
//	AUTHORITY_KIND=...   comma-separated authority kinds (default "ejbca")
package main
...
const (
	connectorID      = "example-legacy-auth-v1"
	connectorName    = "Example Legacy Authority Provider"
...
func main() {
	...
	kinds := strings.Split(envOr("AUTHORITY_KIND", "ejbca"), ",")
	for i := range kinds {
		kinds[i] = strings.TrimSpace(kinds[i])
	}

	store := NewStore()
	handler, err := authority.NewHandler(store,
		authority.Base(handlerbase.WithStrictDecode(envBool("STRICT_DECODE"))),
		authority.WithKinds(kinds...),
	)
	if err != nil {
		logger.Error("build authority v1 handler", "err", err)
		os.Exit(1)
	}
	...
	c, err := shared.New(
		...
		shared.WithInfo(shared.Info{
			...
			Description: "Reference legacy authority connector with an in-memory store. Not for production.",
		}),
